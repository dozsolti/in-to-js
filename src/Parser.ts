...
export class Parser<T extends Record<string, unknown> = {}> {
    ...
    static create(fileContent: Array<string>, i = 0): Parser {
    ...
    array<K extends string, V>(
        name: K,
        splitBy = '',
        mapFunc?: (x: string) => V extends unknown ? unknown : V
    ): Parser<T & { [k in K]: Array<ReturnType<typeof mapFunc>> }> {
        if (!mapFunc)
    ...
    matrix<K extends string, V>(
        name: K,
        length: number | string,
        splitBy = '',
        mapFunc?: (line: string) => V extends unknown ? string : V
    ): Parser<T & {
        [k in K]: Array<Array<ReturnType<typeof mapFunc>>>
    }> {
    ...
    arrayOfObject<K extends string, V extends Parser>(
        name: K,
        length: number | string,
        p: (P: Parser) => V
    ): Parser<T & { [k in K]: Array<ReturnType<typeof p>['obj']> }> {
